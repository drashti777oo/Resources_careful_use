...
export function MainLayout() {
  ...
  const handleQuickAction = (label: string) => {
    if (label === "Add Employee") {
      navigate("/employees", { state: { openAdd: true } })
    } else if (label === "Request Leave") {
      navigate("/leave", { state: { openRequest: true } })
    } else if (label === "Approve Leave") {
      navigate("/leave")
    } else if (label === "Run Payroll") {
      navigate("/payroll")
    }
  }
        ...
        <div className="space-y-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between gap-2">
                  <p className="text-sm font-semibold text-slate-950">{user?.name ?? "Alicia Davis"}</p>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3 text-sm text-slate-700">
              {[
                { label: "Add Employee", icon: Plus, roles: ["ADMIN", "HR", "SUPER_ADMIN"] },
                { label: "Request Leave", icon: CalendarDays, roles: ["EMPLOYEE", "ADMIN", "HR", "MANAGER", "SUPER_ADMIN"] },
                { label: "Approve Leave", icon: ShieldCheck, roles: ["ADMIN", "HR", "MANAGER", "SUPER_ADMIN"] },
                { label: "Run Payroll", icon: Wallet, roles: ["ADMIN", "HR", "SUPER_ADMIN"] },
              ]
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="hidden rounded-full border border-slate-200 bg-white px-3 py-2 shadow-sm sm:flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="text-sm font-semibold text-slate-950">{user?.name ?? "HR Manager"}</p>
                  <p className="text-xs text-slate-500">{user?.role ?? "HR Manager"}</p>
                </div>
              </div>
            </div>
